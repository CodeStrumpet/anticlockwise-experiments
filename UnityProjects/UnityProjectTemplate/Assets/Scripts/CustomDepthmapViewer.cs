...
public class CustomDepthmapViewer : MonoBehaviour 
{
    public Color[] depthColors;
    public float[] depthHeights;
	...
	for (int y = 0; y < YScaled; ++y, i-=XScaled)
		...
		for (int x = 0; x < XScaled; ++x, depthIndex += factor)
			...
			if (pixel == 0)
			    ...
			    {
				byte depthPixel = (byte)depthHistogramMap[pixel];
				float thisDepth = depthPixel / 255.0f;
				Color depthColor = new Color(0,1.0f,0,0);

				bool isBottomLayer = false;
				for (int j = depthHeights.Length - 1; j >= 0; j--) {
				    float depthHeight = depthHeights[j];
				    if (j == 0) {
					isBottomLayer = true;
				    }
				    if (thisDepth > depthHeight) {
					depthColor = depthColors[j];
					break;
				    }
				}
				...
				Color32 c;
				if (isBottomLayer) {
				    c = new Color32((byte)(depthColor.r * 255),
